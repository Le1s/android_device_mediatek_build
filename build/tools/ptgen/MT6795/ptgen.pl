...
=cut
    ...
    {
# Vanzo:wangfei on: Tue, 25 Nov 2014 11:38:19 +0800
# Added for custom partition table
        #$Partition_layout_xls = "$ptgen_location/partition_table_$ArgList{PLATFORM}.xls";
        if(exists $ENV{VANZO_INNER_CUSTOM_PARTITION_TABLE_PATH})
        {
            $Partition_layout_xls = "$ENV{VANZO_INNER_CUSTOM_PARTITION_TABLE_PATH}";
            if( ! -e $Partition_layout_xls){
                my $error_msg = "wyl Custom partition table xls:$Partition_layout_xls does not exist\n";
                print $error_msg;
                die $error_msg;
            }
        }else{
# End of Vanzo:wangfei
            $Partition_layout_xls = "$ptgen_location/partition_table_$ArgList{PLATFORM}.xls";
# Vanzo:wangfei on: Fri, 27 Feb 2015 18:02:41 +0800
# added partition custom 
        }
# End of Vanzo:wangfei
        if ($ArgList{EMMC_SUPPORT} eq "yes")
        {
